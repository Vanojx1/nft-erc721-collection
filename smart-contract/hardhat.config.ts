...
task(
  ...
  async (taskArgs: { newName: string }, hre) => {
    ...
    if (fs.existsSync(newContractFile)) {
      throw Error(`A file with that name already exists: "${oldContractFile}"`);
    }
    ...
    replaceInFile(
      path.join(
        __dirname,
        "/../minting-dapp/src/scripts/lib/NftContractType.ts"
      ),
    ...
    replaceInFile(
      path.join(
        __dirname,
        "/../minting-dapp-vue/src/scripts/lib/NftContractType.ts"
      ),
      CollectionConfig.contractName,
      taskArgs.newName
    );
